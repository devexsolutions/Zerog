    ...
    if doc_type == DocType.DEATH_CERTIFICATE:
        ...
        if date_match:
            try:
                ...
                # Crear fecha asumiendo formato día/mes/año
        ...
        # Patrones: "Don/Doña NOMBRE APELLIDO", "Certifico que NOMBRE APELLIDO"
        name_match = re.search(r'(?:Don|Doña|D\.|Dña\.|Nombre:?)\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)+)', raw_text)
        if name_match:
             extracted_data["deceased_name"] = name_match.group(1)

        # Buscar DNI en certificado de defunción
        ...
        # Si no encuentra fecha, fallback a la fecha actual para no romper el flujo del demo si el OCR falla
