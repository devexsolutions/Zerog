...
USER_EMAIL = f"test_ocr_{random_suffix}@example.com"
...
def create_test_image():
    ...
    # Add text
    # Note: Default font is very small/simple, but Tesseract should read it if it's clear enough.
    # We position it clearly.
    d.text((50, 50), "CERTIFICADO DE DEFUNCION", fill=(0, 0, 0))
    d.text((50, 100), "Nombre: Juan Perez", fill=(0, 0, 0))
    d.text((50, 150), "Fallecio el dia 15-03-2023", fill=(0, 0, 0))
    d.text((50, 200), "En Madrid, Espana", fill=(0, 0, 0))
...
def run_test():
    ...
    # CaseCreate schema requires user_id (str), though backend overrides it.
    case_data = {
        "user_id": "dummy",
        "status": "PENDIENTE",
        "has_will": False
    ...
    print("3. Uploading dummy Death Certificate...")
    img_bytes = create_test_image()
    files = {
        'file': ('death_cert_test.png', img_bytes, 'image/png')
    }
    # DocType enum value for DEATH_CERTIFICATE is 'certificado_defuncion'
    data = {
    ...
    # Wait a moment for OCR to process (it's synchronous in the current implementation but good to be safe)
    time.sleep(2)
    ...
    if date_of_death and "2023-03-15" in date_of_death:
        print("✅ SUCCESS: OCR correctly extracted the date 2023-03-15!")
    else:
